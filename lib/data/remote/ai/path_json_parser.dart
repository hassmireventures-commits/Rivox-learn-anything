...
import '../../../core/ai_platform/output_validator.dart';
...
class PathJsonParser {
  ...
  static GeneratedLearningPath parse(String raw) {
    ...
    final check = validator.validatePathJson(map);
    if (!check.valid) {
      throw InvalidJsonException(check.reason ?? 'Invalid path structure.');
    }

    try {
      return ResourceLinkValidator.validatePath(map);
