...
class ProviderErrorMapper {
  ...
  static AppException map(DioException e, {String providerName = 'AI provider'}) {
    ...
    if (e.type == DioExceptionType.receiveTimeout ||
      ...
      return const GenerationTimeoutException();
    }
    ...
    if (status != null && status >= 500) {
      return ProviderUnavailableException(
        body ?? 'AI provider is temporarily unavailable.',
      );
    }

    return ProviderUnavailableException(
      body ?? 'AI provider is unavailable.',
    );
  }
  ...
  static Duration? _parseRetryAfter(DioException e) {
    ...
    if (raw != null) {
      ...
      try {
        ...
        if (diff.isNegative) return const Duration(seconds: 60);
        return diff;
      } catch (_) {}
    }
