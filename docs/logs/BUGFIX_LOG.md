...
- **Area:** android build config
- **Files:** `android/app/proguard-rules.pro`.
- **Problem / Goal:** Play Console flagged Rivox 1.1.0 (build 7) for obfuscation below its 25% minimum (24%, then 19% on the same build — worse than the previously-published 1.0.5), warning that staying under threshold can affect visibility/publishing on Google Play.
- **Root cause:** `proguard-rules.pro` had blanket rules — `-keep class com.google.firebase.** { *; }`, `-keep class com.google.android.gms.** { *; }`, `-keep class com.google.android.gms.ads.** { *; }`, plus several broad `io.flutter.**` keeps — each preserving *every class and member* in those (large) packages fully unobfuscated and unshrinkable. Firebase/Play Services/GMS make up a large share of a Flutter+Firebase+AdMob app's total class count, so keeping all of them intact directly suppressed the obfuscation percentage; it got worse between 1.0.5 and 1.1.0 simply because this cycle added more Firebase Analytics code, pulling in more fully-kept classes.
- **Solution:** Removed the blanket `-keep` rules for `com.google.firebase.**` / `com.google.android.gms.**` / `com.google.android.gms.ads.**` entirely (kept the `-dontwarn` lines, which only silence build-time warnings and don't affect shrinking). Confirmed safe first: no custom Firestore POJOs, no Crashlytics NDK, and no third-party ad-mediation adapters in this app (only Google's own `google_mobile_ads`) — all of which would have been the reasons an app-level keep might be genuinely necessary. Firebase/Play Services/AdMob all bundle their own consumer ProGuard rules in their AARs, applied automatically by R8 regardless of this file, so nothing actually needed was lost. Narrowed the Flutter keeps to `io.flutter.embedding.**` / `io.flutter.plugin.common.**` / the generated plugin registrant (what the framework actually calls into by exact name from native code) instead of a blanket `io.flutter.**`. Left the MLC LLM/TVM JNI keeps untouched — genuinely required, since on-device LLM native code calls into those classes by exact name.
- **Regression risks:** Real but bounded — this is the standard, well-documented fix for this exact Play Console signal (removing app-level rules that duplicate what libraries already ship), not an experimental change. Verified no in-app usage pattern needs the removed keeps (checked above). Could not complete a live functional smoke test in this environment — the test emulator became unresponsive (stuck package installs, a hung screencap) partway through and was killed rather than fought further.
- **Verified:** Release APK/AAB rebuilt cleanly through R8 with no missing-class errors (a real, if partial, safety signal — R8 fails the build if kept code still references a class the shrinker removed). Split APK sizes dropped ~1.3MB per ABI vs. the unfixed build, confirming R8 is now actually shrinking more, consistent with the fix. Shipped as build 8 (build 7 should not be uploaded — same feature set, superseded solely by this fix). Live device/Play Console pre-launch-report verification still pending on the user's end.
...
- **Area:** router, bootstrap, analytics
- **Files:** `lib/core/router/app_router.dart`, `lib/main.dart`.
- **Problem / Goal:** User-reported the just-shipped release build never got past the native Android icon splash — no crash dialog, just stuck forever. Reproduced live on an emulator; screen capture matched the user's screenshot exactly.
- **Root cause:** Confirmed via direct pub-cache source tracing, not guesswork. `FirebaseAnalyticsObserver(analytics: FirebaseAnalytics.instance)` was added to `appRouter`'s `observers:` list in the prior day's analytics batch. `FirebaseAnalytics.instance` calls `Firebase.app()` **synchronously**, which throws immediately if no Firebase app has been registered yet. `appRouter` is a top-level `final`, evaluated on first access — which happens in `main.dart` at `NotificationService.instance.bindRouter(appRouter)`, *before* `runApp()`. `Firebase.initializeApp()`, however, only runs inside `initializeOptionalServices()`, which `main.dart` deliberately calls `unawaited(...)` *after* `runApp()` (to keep cold start fast). So the very first access of `appRouter` threw synchronously inside `main()`, aborting it before `runApp()` was ever reached — Flutter never painted a first frame, so Android's system splash (the app icon on a plain background) never dismissed. The one-off `[core/no-app]` log line seen in `PlatformDispatcher.instance.onError` was this exact throw; that handler keeps the isolate alive (no crash dialog) but does not resume `main()`'s remaining synchronous code, so `runApp()` genuinely never ran.
- **Solution:** Replaced the eager `FirebaseAnalyticsObserver` with a new `_DeferredAnalyticsObserver` (`app_router.dart`) — a thin `NavigatorObserver` that only constructs the real `FirebaseAnalyticsObserver` lazily, inside each callback, and only once `Firebase.apps.isNotEmpty`; until then every callback silently no-ops. This defers the `FirebaseAnalytics.instance` access to well after `runApp()`, matching the app's existing "never block startup on optional telemetry" convention used elsewhere in `app_bootstrap.dart`. Also reverted an unrelated, unnecessary eager `await ChatFabPositionStore.instance.load()` added to `main.dart` in the same prior-day batch — not the actual root cause, but a real deviation from the established `GuidancePreferencesStore` lazy-load precedent for this exact class of sidecar store, removed as an unnecessary risk on the critical boot path (the FAB already lazily loads it on first build).
- **Regression risks:** None expected — early screen-view events (before Firebase finishes initializing, a window of well under a second) are silently skipped rather than crashing; all subsequent navigation is tracked normally once Firebase is ready.
- **Verified:** Reproduced the exact hang live on `LearnAnything_API36` emulator with the pre-fix release build (screen capture matched user's report). Applied the fix, rebuilt release APK, reinstalled, relaunched — app now reaches the "Welcome to Rivox" onboarding screen normally, no `core/no-app` error in logs. `flutter analyze` (0 new issues, same 35 pre-existing baseline); `flutter test --exclude-tags=live` (180 passed/1 skipped, no regressions).
