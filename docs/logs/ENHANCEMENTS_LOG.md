...
- **Area:** chat, onboarding, dashboard, accounts, learn
- **Files:** `lib/features/dashboard/presentation/dashboard_screen.dart`, `lib/data/remote/ai/chat_service.dart`, `lib/features/chat/presentation/chat_screen.dart`, `lib/l10n/app_en.arb`, `lib/l10n/app_localizations.dart`, all 14 `lib/l10n/app_localizations_*.dart`.
- **Problem / Goal:** User asked to (1) introduce the chat feature once the learner reaches Home post-onboarding rather than during onboarding itself, (2) make chat more agentic and personal, using the learner's name, guiding users, and explaining the app and their own progress ("act like Claude"), (3) check whether Google Sign-In is actually activated, (4) research the YouTube IFrame Player API and validate the app's existing usage, given a past "stuck on play" bug that was the reason chat's own video suggestion uses a plain search link instead of an embed.
- **Findings / Solution:**
  1. Added a one-time dismissible card at the top of the Dashboard (`chat_intro` hint, tracked via the existing `GuidanceController.dismissedHintIds` mechanism) introducing the chat feature with a "Try it" button. This only ever renders once the learner is genuinely on Home; nothing changed in onboarding itself, since the chat FAB already never shows there (see the 2026-09-14 bugfix entry above this one).
  2. Threaded the learner's display name into `ChatService.sendMessage` (fetched from `profileProvider`/`ProfileRepository`, the same source the dashboard greeting already uses) and rewrote the system prompt's opening: warmer tone, uses the learner's name naturally (not forced into every message), and a new `_aboutRivox` reference block giving the model accurate, hand-maintained facts about the app's real features so it can explain them and proactively guide the learner instead of guessing.
  3. **Real, concrete finding, not a code bug:** `android/app/google-services.json`'s `oauth_client` array is empty, meaning no SHA-1/SHA-256 fingerprint is registered in Firebase Console for this app. Google Sign-In cannot work until this is fixed; this is exactly why `kCloudBackupEnabled` has stayed off. Extracted both fingerprints needed to fix it directly from the project's own keystores (`android/app/upload-keystore.jks` for release, `~/.android/debug.keystore` for local testing) via `keytool`, ready to hand to the user (this requires Firebase Console access only they have, not something fixable in code).
  4. Read the official YouTube IFrame Player API docs and cross-checked the app's existing `InAppYoutubePlayer` (used for daily content/paths, `youtube_player_flutter` built on `youtube_player_iframe`): it already follows the documented requirements (`playsInline`, `enableJavaScript`, `showControls`, the last one itself a fix for a real prior playback bug per `docs/logs/BUGFIX_LOG.md`'s 2026-07-22 entry) and already has an error-to-search-fallback. The `origin: 'https://www.youtube.com'` parameter, initially suspicious, turned out to be the package's own documented default for non-web platforms after checking its source, not a bug. No further code change made here; chat's own choice to use a plain external search link rather than an embed was confirmed as the safer, already-correct call given the documented embed-bug history, not changed.
- **Regression risks:** None expected for the dashboard hint (additive, dismissible, same pattern as the existing "add provider" prompt) or the chat prompt changes (system prompt text only, no schema/parsing change).
- **Verified:** `flutter analyze` (0 new issues, 35 pre-existing baseline); `flutter test --exclude-tags=live` (209 passed/1 skipped, unchanged count since this batch didn't add new testable logic beyond prompt text and UI).
